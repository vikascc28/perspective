...
namespace computed_function {
    ...
    const std::string days_of_week[7] = {"1 Sunday", "2 Monday", "3 Tuesday",
        "4 Wednesday", "5 Thursday", "6 Friday", "7 Saturday"};

    const std::string months_of_year[12] = {"01 January", "02 February",
    ...
    bucket::operator()(t_parameter_list parameters) {
        ...
        if (val.is_numeric()) {
            ...
            rval.set(
                floor(val.to_double() / unit.to_double()) * unit.to_double());
            ...
            switch (date_bucket_unit) {
                ...
                default: {
                    PSP_COMPLAIN_AND_ABORT(
                        "[bucket] invalid date bucket unit!");
                } break;
            }
        } else {
