...
extern "C" int EMSCRIPTEN_KEEPALIVE cadd(int x,int y) {
    t_date d(2020, 4, 10);
    return d.month();
}
...
//      */
    ...
    make_table() {
        ...
        column_names =  { "x", "y" }; 
        data_types = { DTYPE_INT32, DTYPE_STR };
        ...
        // } else {
        // row_count = 2; //accessor["row_count"].as<std::int32_t>();
        // }
        ...
        // } else {
        // _fill_data(data_table, accessor, input_schema, NULL, 0, std::numeric_limits<std::uint32_t>::max(), false);
        // }
        ...
        tbl->init(data_table, row_count, OP_INSERT, 1);
        return tbl->size();
    }
