...
t_stree::t_stree(const std::vector<t_pivot>& pivots,
...
t_stree::build_strand_table_phase_1(t_tscalar pkey, t_op op, t_uindex idx,
    ...
    for (t_uindex pidx = 0, ploop_end = pivot_like.size(); pidx < ploop_end;
        ...
        const std::uint8_t* trans_
            = piv_tcols[pidx]->get_nth<std::uint8_t>(idx);
    ...
    if (op == OP_DELETE) {
        ...
        strand_count
            = npivots == 0 || !no_new_rows || pivots_neq || force_current_row
            ? 1
            : 0;
    }
        ...
        for (const auto& dep : aggspec.get_dependencies()) {
            if (dep.type() == DEPTYPE_COLUMN) {
                ...
                if (aggspec.is_non_delta()
                    ...
                    metadata.m_strand_schema.add_column(depname,
                        metadata.m_flattened_schema.get_dtype(depname));
    ...
    if (has_filters) {
        for (t_uindex idx = 0, loop_end = flattened.size(); idx < loop_end;
            ...
            if (!filter_prev && !filter_curr) {
                ...
                // apply current row
                build_strand_table_phase_1(pkey, op, idx, metadata.m_pivsize,
                    strand_count_idx, aggcolsize, true, piv_ccols, piv_tcols,
                    agg_ccols, agg_dcols, piv_scols, agg_acols, agg_scount,
                    spkey, insert_count, pivots_neq,
                    metadata.m_pivot_like_columns);
            } else if (filter_prev && !filter_curr) {
                // reverse prev row
                build_strand_table_phase_2(pkey, idx, metadata.m_pivsize,
        ...
        for (t_uindex idx = 0, loop_end = flattened.size(); idx < loop_end;
            ...
            build_strand_table_phase_1(pkey, op, idx, metadata.m_pivsize,
                ...
                insert_count, pivots_neq, metadata.m_pivot_like_columns);
...
t_stree::update_aggs_from_static(const t_dtree_ctx& ctx, const t_gstate& gstate,
    ...
    for (const auto& r : m_tree_unification_records) {
        ...
        update_agg_table(r.m_sptidx, agg_update_info, r.m_daggidx, r.m_saggidx,
            r.m_nstrands, gstate, expression_master_table);
    }
}
    ...
    for (t_uindex idx : info.m_dst_topo_sorted) {
        ...
        switch (spec.agg()) {
            ...
            case AGGTYPE_UNIQUE: {
                ...
                bool is_unique
                    = is_unique_from_gstate(gstate, expression_master_table,
            ...
            case AGGTYPE_JOIN: {
                ...
                new_value.set(reduce_from_gstate<
                    std::function<t_tscalar(std::vector<t_tscalar>&)>>(gstate,
                    expression_master_table, spec.get_dependencies()[0].name(),
                    pkeys, [this](std::vector<t_tscalar>& values) {
            ...
            case AGGTYPE_LAST_VALUE: {
                ...
                if (iters.first != iters.second) {
                    ...
                    dst->set_scalar(dst_ridx,
                        read_by_pkey_from_gstate(gstate,
                            expression_master_table,
                            spec.get_dependencies()[0].name(), pkey));
                } else {
                    dst->set_scalar(dst_ridx, mknone());
                }
            } break;
...
void
t_stree::read_column_from_gstate(const t_gstate& gstate,
...
t_tscalar
t_stree::read_by_pkey_from_gstate(const t_gstate& gstate,
    const t_data_table& expression_master_table, const std::string& colname,
...
bool
t_stree::is_unique_from_gstate(const t_gstate& gstate,
    const t_data_table& expression_master_table, const std::string& colname,
...
bool
t_stree::apply_from_gstate(const t_gstate& gstate,
    const t_data_table& expression_master_table, const std::string& colname,
...
t_stree::reduce_from_gstate(const t_gstate& gstate,
    const t_data_table& expression_master_table, const std::string& colname,
