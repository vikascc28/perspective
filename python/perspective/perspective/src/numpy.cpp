...
namespace numpy {
    ...
    void
    NumpyLoader::fill_column(t_data_table& tbl, std::shared_ptr<t_column> col,
    ...
    template <typename T>
    void
    NumpyLoader::fill_object_iter(t_data_table& tbl,
    ...
    template <>
    void
    NumpyLoader::fill_object_iter<std::int32_t>(t_data_table& tbl,
        std::shared_ptr<t_column> col, const std::string& name,
        t_dtype np_dtype, t_dtype type, std::uint32_t cidx, bool is_update) {
        t_uindex nrows = col->size();
    ...
    template <>
    void
    NumpyLoader::fill_object_iter<std::int64_t>(t_data_table& tbl,
        std::shared_ptr<t_column> col, const std::string& name,
        t_dtype np_dtype, t_dtype type, std::uint32_t cidx, bool is_update) {
        t_uindex nrows = col->size();
    ...
    template <>
    void
    NumpyLoader::fill_object_iter<double>(t_data_table& tbl,
        std::shared_ptr<t_column> col, const std::string& name,
        t_dtype np_dtype, t_dtype type, std::uint32_t cidx, bool is_update) {
        t_uindex nrows = col->size();
    ...
    void
    NumpyLoader::fill_column_iter(const py::array& array, t_data_table& tbl,
        std::shared_ptr<t_column> col, const std::string& name,
        t_dtype np_dtype, t_dtype type, std::uint32_t cidx, bool is_update) {
        PSP_VERBOSE_ASSERT(m_init, "touching uninited object");
    ...
    void
    NumpyLoader::fill_datetime_iter(const py::array& array, t_data_table& tbl,
        std::shared_ptr<t_column> col, const std::string& name,
        t_dtype np_dtype, t_dtype type, std::uint32_t cidx, bool is_update) {
        PSP_VERBOSE_ASSERT(m_init, "touching uninited object");
        t_uindex nrows = col->size();
    ...
    void
    NumpyLoader::fill_date_iter(std::shared_ptr<t_column> col,
    ...
    NumpyLoader::fill_numeric_iter(const py::array& array, t_data_table& tbl,
        ...
        if (types_mismatched || np_dtype == DTYPE_OBJECT) {
            switch (type) {
                ...
                default:
                    PSP_COMPLAIN_AND_ABORT("Unable to fill non-numeric column `"
                        + name + "` in `fill_numeric_iter`.")
            }
        }
     ...
     */
    t_fill_status
    NumpyLoader::try_copy_array(const py::array& src,
    ...
    void
    NumpyLoader::fill_validity_map(std::shared_ptr<t_column> col,
    ...
    template <typename T>
    void
    copy_array_helper(const void* src, std::shared_ptr<t_column> dest,
        const std::uint64_t offset) {
    ...
    NumpyLoader::make_types() {
        ...
        for (auto i = 0; i < m_names.size(); ++i) {
            ...
            if (!array) {
                PSP_COMPLAIN_AND_ABORT("Perspective does not support the "
